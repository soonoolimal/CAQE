...
class BaseHiddenExtractor(ABC):
    def __init__(self, backbone_key: str, data_cfg: dict):
        ...
        self.save_dir = Path(save_cfg["dirs"]["hidden_vecs"]) / backbone_key
    ...
    def _load_llm_and_tokenizer(self):
        print(f"[{self.backbone_key}] Loading model: {self.model_hf_id}")
        ...
        self.llm = AutoModel.from_pretrained(self.model_hf_id, dtype=torch.bfloat16)
        self.llm.eval()
    ...
    def _save_hidden_chunk(self, data: dict, chunk_idx: int):
        ...
        tqdm.write(f"[{self.backbone_key}] Chunk saved: {save_path} | shape={tuple(data['hidden'].shape)}")
    ...
    def _save_metadata(self, h_dim: int, backbone_type: str):
        ...
        print(f"[{self.backbone_key}] Metadata saved: {save_path}")
...
class MLMHiddenExtractor(BaseHiddenExtractor):
    ...
    def _make_masked_inputs(
        ...
        """Create a masked copy of each content token's sentence with that token replaced by [MASK]."""
        ...
        # i: sentence index in current batch
    ...
    def _forward_masked(self, masked_inputs: dict) -> torch.Tensor | None:
        """Extract the hidden vector at each [MASK] position.

        This is the contextual representation from surrounding context, not the token embedding itself.
        """
        ...
        if not masked_input_ids:
            tqdm.write(f"[{self.backbone_key}] Warning: no content tokens found in batch, skipping")
            return None
        ...
        for start in range(0, len(masked_input_ids), self.fwd_bs):
            ...
            outputs = self.llm(input_ids=batch_ids, attention_mask=batch_mask)
    ...
    def extract(self, texts: list[str]):
        """Extract hidden vectors for all content tokens in texts and save them as chunked .pt files.
        ...
        if self.save_dir.exists():
            print(f"[{self.backbone_key}] Output dir already exists, skipping: {self.save_dir}")
            return
        ...
        pbar = tqdm(range(0, len(texts), self.tok_bs), desc=f"MLM [{self.backbone_key}]", dynamic_ncols=True)
        ...
        self._save_metadata(h_dim=self.llm.config.hidden_size, backbone_type="mlm")
        self.llm = None
...
class NTPHiddenExtractor(BaseHiddenExtractor):
    ...
    def _make_ntp_pairs(
        ...
        """Record the target token and hidden extraction position t-1 for each content token at position t."""
        special_ids = set(self.tokenizer.all_special_ids)
        ...
        for i in range(batch_size):
            for pos in range(1, seq_len):
                ...
                batch_indices.append(i)  # sentence index within tokenizer batch
    ...
    def _forward_batch(self, pairs: dict) -> torch.Tensor | None:
        """Run a forward pass and extract h_{t-1} for each content token at position t."""
        assert self.llm is not None
        all_hidden = []
        ...
        for start in range(0, tok_bs, self.fwd_bs):
            ...
            # rows: sentence index within sub-batch
            # cols: h_{t-1} position of each pair
            rows = torch.tensor([batch_indices[k] - start for k in sel], device=self.device)
            cols = torch.tensor([hidden_positions[k] for k in sel], device=self.device)
            all_hidden.append(last_hidden_states[rows, cols].detach().cpu())
    ...
    def extract(self, texts: list[str]):
        ...
        pbar = tqdm(range(0, len(texts), self.tok_bs), desc=f"NTP [{self.backbone_key}]", dynamic_ncols=True)
        for batch_start in pbar:
            batch_texts = texts[batch_start : batch_start + self.tok_bs]
            input_ids, attention_mask = self._tokenize(batch_texts)
        ...
        self._save_metadata(h_dim=self.llm.config.hidden_size, backbone_type="ntp")
        self.llm = None
        torch.cuda.empty_cache()
